...
export function HistoryPanel({ activities, onRestoreDocument, onRestoreThread }: HistoryPanelProps) {
  ...
  const allActivities = isAuthenticated ? storedActivities : activities;
  const filtered = useMemo(() => allActivities.filter(item => filter === "all" || item.kind === filter), [allActivities, filter]);
